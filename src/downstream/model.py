...
class GOAT_geneEmbedding(nn.Module):
    def __init__(self, net_params, **kwargs):
        ...
        self.conv_layers = nn.ModuleList([GraphTransformerLayer(
            hidden_dim, hidden_dim, num_heads, dropout, residual, batch_norm, layer_norm) for _ in range(num_layers-1)])
        self.conv_layers.append(GraphTransformerLayer(hidden_dim, output_dim,
                                num_heads, dropout, residual, batch_norm, layer_norm))
...
class MLP_embedding(nn.Module):
    def __init__(self, net_params):
        super().__init__()
        self.num_nodes = net_params['num_nodes']  # number of nodes in a graph
        self.input_dim = net_params['input_dim']  # input dimension of a node
        dropout = net_params['dropout']
        patient_dim = self.input_dim * self.num_nodes
        self.output_dim = patient_dim // 16  # output dimension calculated here

        self.layer1 = nn.Linear(patient_dim, self.output_dim)
...
class classifier(nn.Module):
    ...
    def forward(self, h):
        h = F.selu(self.layer1(h))
        h = self.dropout_layer(h)
        h = F.selu(self.layer2(h))
        h = self.dropout_layer(h)
        h = F.selu(self.layer3(h))
        return h
...
class MLP(nn.Module):
    def __init__(self, net_params, **kwargs):
        ...
        output_dim_embedding = self.patient_embedding.output_dim  # Get output dim from MLP_embedding
        dropout = net_params['dropout']
        self.classifier = classifier(output_dim_embedding, 1, dropout)  # Correct input dimension for classifier
        self.dropout_layer = nn.Dropout(p=dropout)
