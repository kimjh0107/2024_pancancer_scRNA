...
class patientDGL_multiomics(Dataset):
    def __init__(self, data_dir):
        ...
        topology_file = glob(data_dir+'/../STRING_human.tsv')
...
class patientDGL_singleomics(Dataset):
    def __init__(self, data_dir):
        ...
        # NaN 값을 0으로 대체하거나 다른 값으로 대체
        for data in data_tmp:
            data['omics'] = data['omics'].fillna(0)
            data['label'] = np.nan_to_num(data['label'])
            
        patient_ids = [data['omics'].index[0] for data in data_tmp]
        patient_id_idx = pd.DataFrame(enumerate(patient_ids), columns=[
                                      'idx', 'patient_id']).sort_values(by='patient_id')['idx'].to_numpy()
        ...
        # NaN 값을 포함하는 엣지를 제거
        self.topology = self.topology.dropna()
    ...
    def _prepare(self):
        for omics in self.data:
            node_features = torch.reshape(torch.tensor(
                omics['omics'].values, dtype=torch.float64), (-1, 1))
            num_nodes = len(node_features)
            g_nodeID = dict(zip(omics['omics'].columns, range(num_nodes)))

            # edges = self.topology.iloc[:, [0, 1]].map(
            #     lambda x: g_nodeID[x] if x in g_nodeID else None).dropna().to_numpy()
            edges = self.topology.iloc[:, [0, 1]].applymap(
                lambda x: g_nodeID[x] if x in g_nodeID else None).dropna().to_numpy()
            srcs, tgts = torch.tensor(edges[:, 0]), torch.tensor(edges[:, 1])
            g = dgl.graph((srcs, tgts), num_nodes=num_nodes,
                          idtype=torch.int64)
...
def laplacian_positional_encoding(g, pos_enc_dim):
    ...
    N = sparse.diags(dgl.backend.asnumpy(
        g.in_degrees()).clip(1) ** -0.5, dtype=float)
    # <- Symetrically normalized Laplacian
